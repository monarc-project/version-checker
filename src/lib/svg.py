...
def simple_text(name, style, text=None):
    ...
    paragraph = dwg.add(
        dwg.g(font_size=14, font_family="DejaVu Sans", fill=style["paragraph_fill"])
    )
    if not text:
        text = style["text"]
    text_elem = dwg.text(
        text, insert=(5, 15), font_weight="bold", fill=style["text_fill"]
    )
...
if __name__ == "__main__":
    for state in ["up-to-date", "update available", "security update available"]:
        simple_text("{}.svg".format(state.replace(" ", "-")), state, STYLE[state])
